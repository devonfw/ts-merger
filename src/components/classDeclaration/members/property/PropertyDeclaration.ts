...
export class PropertyDeclaration extends GeneralInterface {
    ...
    merge(patchProperty: PropertyDeclaration, patchOverrides: boolean) {
        ...
        mergeTools.mergeDecorators(this.getDecorators(), patchProperty.getDecorators(), patchOverrides);

        if(this.getInitializer() instanceof ObjectLiteralExpression && patchProperty.getInitializer() instanceof ObjectLiteralExpression) {
            (<ObjectLiteralExpression>this.getInitializer()).merge((<ObjectLiteralExpression>patchProperty.getInitializer()), patchOverrides);
        }else if(this.getInitializer() instanceof ArrayLiteralExpression && patchProperty.getInitializer() instanceof ArrayLiteralExpression) {
            (<ArrayLiteralExpression>this.getInitializer()).merge((<ArrayLiteralExpression>patchProperty.getInitializer()), patchOverrides);
        } else if(this.getInitializer() instanceof CallExpression && patchProperty.getInitializer() instanceof CallExpression) {
            (<CallExpression>this.getInitializer()).merge((<CallExpression>patchProperty.getInitializer()), patchOverrides);
