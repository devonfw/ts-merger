...
export class Decorator extends GeneralInterface{
    ...
    private args: any[] = [];
    private isCallExpression: boolean = false;
    ...
    merge(patchDecorator: Decorator, patchOverrides) {
        ...
        patchDecorator.getArguments().forEach(patchArgument => {
            this.getArguments().forEach(argument => {
                if(patchArgument instanceof ObjectLiteralExpression && argument instanceof ObjectLiteralExpression) {
                    (<ObjectLiteralExpression>argument).merge((<ObjectLiteralExpression>patchArgument), patchOverrides);
                } else if(patchArgument instanceof ArrayLiteralExpression && argument instanceof ArrayLiteralExpression) {
                    (<ArrayLiteralExpression>argument).merge(<ArrayLiteralExpression>patchArgument, patchOverrides);
